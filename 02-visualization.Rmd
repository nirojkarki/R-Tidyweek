...
# color aesthetic
ggplot(data = bechdel) +
    geom_point(mapping = aes(x =budget  , y =domgross , color = "red", shape= binary), alpha=1)
...
# shape aesthetic
ggplot( data = bechdel) +
    geom_point(mapping = aes(x = budget , y = domgross , shape = binary ))
```
...
# alpha aesthetic 
ggplot( data = bechdel) +
    geom_point(mapping = aes(x = budget , y = domgross , alpha = 0.8 ))
```
...
```{r echo=FALSE}
ggplot(data = bechdel) +
  geom_histogram(mapping = aes(x = intgross ), binwidth = 100000000)
```
...
ggplot(data = bechdel,aes(x = clean_test))+
  geom_bar(aes(fill = clean_test))+labs(title = "Bar Graph", x="Count", y="Clean test", caption="Data Source: Bechel")
```
...
bechdel %>% count(test) %>% ggplot() +
    geom_col(aes(x=test,y=n,fill= test)) +
...
ggsave("myplot.png", plot = myplot , width =  6, height = 6, units = "in", dpi = 600)
```
