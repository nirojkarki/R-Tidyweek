...
```{r}
ttest_max_heart_beat <- t.test(heart$max_heart_rate, mu = 90)
ttest_max_heart_beat
```
...
```{r}
chisq_chest_heart$observed %>% knitr::kable(format = "html")
```
...
ggplot(heart)+
  geom_smooth(aes(age,max_heart_rate), method = "lm", se=FALSE)+
  geom_point(aes(age,max_heart_rate))+
  theme_minimal()
```
